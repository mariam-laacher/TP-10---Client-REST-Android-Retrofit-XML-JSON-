...
class MainActivity : AppCompatActivity(), OnDeleteClickListener, OnUpdateClickListener {
    ...
    private var radioJson: RadioButton? = null
    private var radioXml: RadioButton? = null
    private var addbtn: FloatingActionButton? = null
    private var currentFormat: String = "JSON"
    ...
    private fun setupFormatSelection() {
        ...
        // Définir JSON comme sélectionné par défaut
        radioJson!!.isChecked = true
        radioXml!!.isChecked = false
    }
        ...
        compteRepository.addCompte(compte, object : Callback<Compte?> {
            override fun onResponse(call: Call<Compte?>, response: Response<Compte?>) {
                if (response.isSuccessful) {
                    ...
                    showToast("Erreur lors de l'ajout: ${response.code()} - ${response.message()}")
                    if (response.errorBody() != null) {
                        showToast("Détails: ${response.errorBody()?.string()}")
                    }
                }
            }

            override fun onFailure(call: Call<Compte?>, t: Throwable) {
                showToast("Erreur lors de l'ajout: ${t.message}")
                t.printStackTrace()
            }
        })
    }
