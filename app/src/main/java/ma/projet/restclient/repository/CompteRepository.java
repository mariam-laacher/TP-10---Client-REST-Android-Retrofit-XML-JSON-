...
public class CompteRepository {
    ...
    public void addCompte(Compte compte, Callback<Compte> callback) {
        Call<Compte> call;
        if ("JSON".equals(format)) {
            call = compteService.addCompteJson(compte);
        } else {
            call = compteService.addCompteXml(compte);
        }
        call.enqueue(callback);
    }
    ...
    public void updateCompte(Long id, Compte compte, Callback<Compte> callback) {
        Call<Compte> call;
        if ("JSON".equals(format)) {
            call = compteService.updateCompteJson(id, compte);
        } else {
            call = compteService.updateCompteXml(id, compte);
        }
        call.enqueue(callback);
    }
